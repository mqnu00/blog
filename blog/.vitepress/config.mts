...
export default defineConfig({
  ...
  async buildEnd(siteConfig) {
    ...
    const feed = new Feed({
      ...
      copyright: "© 2025 广习习",
      updated: moment.utc(new Date()).utcOffset(8).toDate(),
    });
  ...
  async transformPageData(pageData) {
    ...
    const history = await getGithubHistory({
      ...
      res.forEach((item) => {
        item.date =
          moment.utc(item.date).utcOffset(8).format("YYYY-MM-DD HH:mm:ss Z") ??
          null;
      });
      return res;
    });
